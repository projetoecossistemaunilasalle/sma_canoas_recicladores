...
export const publications = pgTable("publications", {
  ...
  // Cooperativa responsável pela publicação
  ...
  importantPhotoUrl: text("important_photo_url"),

  importantPhotoDescription: text(
    "important_photo_description"
  ),
  ...
  updatedPhotoUrl: text("updated_photo_url"),

  updatedPhotoDescription: text(
    "updated_photo_description"
  ),

  photoUpdatedAt: date("photo_updated_at"),
...
export const users = pgTable("users", {
  ...
  cooperativeId: uuid("cooperative_id").references(
    () => cooperatives.id
  ),
...
export const vehiclePositions = pgTable("vehicle_positions", {
  ...
  vehicleId: uuid("vehicle_id").references(
    () => vehicles.id,
    {
      onDelete: "cascade",
    }
...
export const collectionRoutes = pgTable(
  "collection_routes",
  {
    ...
    totalDistanceKm: doublePrecision(
      "total_distance_km"
    ),

    totalDurationSeconds: doublePrecision(
...
export const routeStreets = pgTable("route_streets", {
  ...
  direction: text("direction").default("forward"),

  distanceFromPreviousKm: doublePrecision(
    "distance_from_previous_km"
  ),

  durationFromPreviousSeconds: doublePrecision(
    "duration_from_previous_seconds"
  ),

  // true for streets the cooperative actually picked as stops
  // false for connector segments
  isStop: boolean("is_stop")
    .notNull()
    .default(false),
})
...
export const cooperativesRelations = relations(
  cooperatives,
  ({ many }) => ({
    users: many(users),
...
export const publicationsRelations = relations(
  ...
  ({ one }) => ({
    cooperative: one(cooperatives, {
      fields: [publications.cooperativeId],
...
export const routeStreetsRelations = relations(
  routeStreets,
  ({ one }) => ({
...
export type Cooperative =
  typeof cooperatives.$inferSelect

export type NewCooperative =
  typeof cooperatives.$inferInsert
...
export type VehiclePosition =
  typeof vehiclePositions.$inferSelect

export type NewVehiclePosition =
  typeof vehiclePositions.$inferInsert

export type Street =
  typeof streets.$inferSelect

export type NewStreet =
  typeof streets.$inferInsert

export type CollectionRoute =
  typeof collectionRoutes.$inferSelect

export type NewCollectionRoute =
  typeof collectionRoutes.$inferInsert

export type RouteStreet =
  typeof routeStreets.$inferSelect

export type NewRouteStreet =
  typeof routeStreets.$inferInsert